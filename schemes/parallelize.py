...
class PaillierScheme:
    ...
    @staticmethod
    def compute_ct1(message: int, precomputed_gm: dict, nsquared: int):
        # Split message into two 16-bits numbers
        j0 = (message // (POWER ** 0)) % POWER
        j1 = (message // (POWER ** 1)) % POWER

        gm = (
            precomputed_gm["1"][str(j1)] * precomputed_gm["0"][str(j0)]
        ) % nsquared

        return gm

    @staticmethod
    def compute_ct2(precomputed_gnr: list, nsquared: int):
        # Get NO_GNR random precomputed (g^n)^r and
        # multiply them with each other
        gnr = reduce(mul, random.sample(precomputed_gnr, NO_GNR), 1) % nsquared
        return gnr
    ...
    def encrypt(self, message: int) -> int:
        ...
        with multiprocessing.Pool(2) as pool:
            ...
            p1 = pool.apply_async(
                self.compute_ct1,
                (message, self.precomputed_gm, self.public.nsquared),
            )

            p2 = pool.apply_async(
                self.compute_ct2,
                (self.precomputed_gnr, self.public.nsquared),
            )

            ciphertext = (p1.get() * p2.get()) % self.public.nsquared
